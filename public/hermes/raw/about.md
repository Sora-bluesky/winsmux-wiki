---
title: このサイトについて
description: 運営者、データの扱い、免責。プライバシーポリシーを兼ねます。
sources:
  - https://hermes-agent.nousresearch.com/docs/
hermes_version: "0.20.5"
confidence: high
raw: /hermes/raw/about.md
---

# このサイトについて

Hermes Agent Wiki は、[Hermes Agent](https://github.com/NousResearch/hermes-agent) の公式ドキュメントの日本語版と、逆引き・トラブル対処などの独自ページをのせた個人運営のサイトです。開発元の NousResearch とは無関係の**非公式**サイトで、正本は常に [公式ドキュメント](https://hermes-agent.nousresearch.com/docs/) です。

- 運営者: そら（X: [@sora_biz](https://x.com/sora_biz) / GitHub: [Sora-bluesky](https://github.com/Sora-bluesky)）
- 問い合わせ: 各ページ下部の「このページへの意見・要望」から（GitHub の Issue として届きます）

## データの扱い（プライバシーポリシー）

このサイト自身は、個人情報を集めていません。

- アクセス解析ツール・広告は使っていません
- cookie は使っていません
- 入力フォームはありません。意見・要望は GitHub 上の Issue フォームで受けており、そこでの入力には [GitHub のプライバシーステートメント](https://docs.github.com/ja/site-policy/privacy-policies/github-privacy-statement) が適用されます
- 「AI に渡す」バーで選んだ送り先は、ブラウザ内（localStorage）にだけ保存されます。外部には送られません

閲覧にともなって、次の外部サービスへ接続情報（IP アドレスなど）が送られます。

- [Cloudflare](https://www.cloudflare.com/ja-jp/privacypolicy/) — このサイトの配信に使っており、アクセスログが同社の設備に残ります
- [Google Fonts](https://policies.google.com/privacy?hl=ja) — 文字の表示に使うフォントを Google のサーバーから読み込みます

## 免責と著作権

- 内容の正確性には注意していますが、保証はしません。食い違いがあれば公式ドキュメントが正です
- 翻訳元である公式ドキュメントの権利は NousResearch に帰属します（[MIT License](https://github.com/NousResearch/hermes-agent/blob/main/LICENSE)）
- 誤りを見つけたら [Issue フォーム](https://github.com/Sora-bluesky/winsmux-wiki/issues/new?template=feedback.yml) から知らせてもらえると助かります
