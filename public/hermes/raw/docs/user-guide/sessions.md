...
3. ゲートウェイの監視役が引き継ぎを引き受け、移動先のアダプターに新しいスレッドを作らせます。
   ...
   - **WhatsApp / Signal / SMS** — スレッドのしくみがないので、ホームチャンネルへ直接送ります。
...
**知っておきたい制限:** スレッドに対応していないプラットフォームで、ホームチャンネルが複数人のグループになっている場合、合成のターンは DM 形式のセッションとしてキーが作られます。自分だけの DM をホームチャンネルにする一般的な使い方では問題ありませんが、本当に共有しているグループチャットには向きません。スレッドが使えるのは Telegram / Discord / Slack / Matrix で、これがほとんどの場合に当たるため、この制限に触れることはまずありません。
...
- 動作中のターンは、それを始めた送信者のキーに結び付きます。ただし `/stop` はそのターンにも届きます。詳しくは下記を参照してください

`/stop` は「このチャットで動いているものを止める」という意味です。まず呼び出した人自身の
セッションキーを試し、次にこのチャットで動いているターン（呼び出した人自身のスレッドにある
ほかの参加者の実行も含みます）を対象にします。権限の確認は入りますが、別の部屋や
ワークスペース、プロファイルには決して及びません。つまり、何もしていないアリスの `/stop` が、
同じ部屋でボブ（あるいはボット）が始めたターンを終わらせることがあります。スレッドの*中から*
送った `/stop` はもう少し狭く、そのスレッドに属する実行と、スレッドの枠を持たない部屋全体の
実行（DM が流れていく形のもの）に届きますが、同じチャンネルの別のスレッドや、ほかの人の
送信者ごとのトップレベルの実行には届きません。
...
および `hermes sessions recover` が迷子のメッセージのために作る `recovered` の
仮の行）の開きっぱなしのセッションのうち、最後の動きが `retention_days` より
古いものを*閉じ*ます（`end_reason: startup_orphan_reap`）。閉じるだけなら何も失われず、
