...
upstream_blob: f90ea00f015bc787af63d6b6755fb18805b4546c
sources:
...
### セッションのリセットの決め方 {#session-reset-policies}

**既定では、ゲートウェイのセッションは自動でリセットされません**（`mode: none`）。
自動のリセットを使いたい場合は、`config.yaml` の `session_reset` の節で選びます。

- **none** — 自動リセットなし（既定。文脈は `/reset` と圧縮で管理します）
- **idle** — N 分間なにも起きなければリセット
- **daily** — 毎日、決まった時刻にリセット
- **both** — 先に来たほう（idle か daily）でリセット

セッションが自動でリセットされる前に、エージェントには、その会話から大事な記憶や skill を保存する機会が 1 ターン与えられます。

**バックグラウンドの処理が動いている**セッションは、設定にかかわらず自動でリセットされません。
...
- ゲートウェイのセッションは、設定したリセットの決まりに従って自動でリセットされます
