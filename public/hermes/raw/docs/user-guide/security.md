...
upstream_blob: a5f619d4fff1c824b4e224fe96be5edd83af9a83
sources:
...
- パターンは [fnmatch](https://docs.python.org/3/library/fnmatch.html) のグロブ（`*`、`?`、`[...]`）で、コマンド全体に対して **大文字小文字を区別せずに** 照合します。`git push --force*` は `git push --force origin main` に当たりますが、`git push origin main` には当たりません。
...
- 拒否ルールが効くのは、ホストに手が届くバックエンド（local、SSH、ホストをマウントした Docker）です。隔離されたコンテナのバックエンドは、以前からそうであるように、この一連の防御を丸ごと飛ばします。その中で動くものはホストに触れないからです。
...
:::note 想定している脅威
拒否ルールは、悪気はないけれど間違えるエージェントに対する安全策で、危険なパターンの検出器と同じ想定です。はっきり敵対してくるプロセスに対するサンドボックスではありません。そちらには隔離されたバックエンド（Docker、Modal）や、外向きの通信を絞った環境を使ってください。
:::
