...
## TLS 証明書の信頼 {#tls-certificate-trust}

Hermes は `truststore` を通してプラットフォームの検証機構を初期化します。Windows では
その証明書ストアを、macOS ではシステムの信頼サービスを、Linux では
OpenSSL のシステムの信頼パスを使います。初期化に失敗した場合は、その失敗をログに残し、
OpenSSL の既定に戻ります。

社内の TLS プロキシを使う場合は、そのルート証明書を、組織が定める OS の信頼設定の手順で入れてください。
Hermes のプロバイダーの解決処理は、もう `HERMES_CA_BUNDLE` や、以前の CA 用の環境変数を順に見る仕組みで
信頼先を選びません。サンドボックスの中のサブプロセスは、それとは別の CA 設定を持てます。

これに伴い、以前あった起動時の証明書の確認も無くなりました。Hermes は起動時に
`HERMES_CA_BUNDLE` / `SSL_CERT_FILE` / `REQUESTS_CA_BUNDLE` /
`CURL_CA_BUNDLE` を確かめなくなったので、`SSLConfigurationError` は出ず、
逃げ道だった `HERMES_SKIP_SSL_GUARD` も効きません。`HERMES_CA_BUNDLE` は、
Nous Portal へのログインの流れ（`hermes login`、またはその
`--ca-bundle` フラグ）でだけ今も使われます。標準の `SSL_CERT_FILE` / `REQUESTS_CA_BUNDLE` / `CURL_CA_BUNDLE` の変数は、
一部のツールが行う素の `requests`/`urllib` の呼び出し（と、
`pip`、`uv`、`curl`、Node）が今も読みます。そのため、これらのどれかに古いパスが残っていると、起動時ではなく
それを使う呼び出しの時点で失敗します。その場で変数を直すか、解除してください。

カスタムのプロバイダーは、自分のエンドポイント用に `ssl_ca_cert` を宣言できます。その証明書の束は、
チャット、モデルのメタデータ、モデル一覧の問い合わせで、プラットフォームの信頼の代わりに使われます。
ファイルが無い場合は警告を出し、プラットフォームの信頼に戻ります。
`ssl_verify: false` は証明書の検証を止めるもので、信頼できないネットワークでは危険です。
社内のルート証明書が足りないことへの恒久的な対処として使わないでください。

プロバイダーの HTTP クライアントは、プロキシの設定と証明書の選択を分けて扱います。
環境に残った古い CA ファイルのパスがあっても、それらのクライアントの起動は妨げられません。
PM のインデックスの認証情報は、完全に一致する HTTPS のオリジンにだけ送られます。
別のオリジンへのリダイレクト先には渡りません。
...
PM は、任意の Python の機能を `pyproject.toml` の追加指定（extras）として管理します。
ソースからのインストーラーは `all` の追加指定を選びます。ネイティブの同梱版には、その環境が対応する
すべての追加指定が入っています。この 2 つは別の機能の組み合わせです。

バックエンドが使えない追加指定を求めると、`pm.ensure_import("extra-name")` が
プラグインの受け入れと同じ依存関係の処理を使います。

1. PM は、プラットフォームが対応しているかと `security.allow_lazy_installs` を確かめます。
2. PM は、既存の追加指定と、有効なプラグインの要件をすべて含んだ環境を用意します。
3. プラグインのメンバーが無ければ、コミット済みのロックをそのまま使います。メンバーがあれば、前回の選択をもとに解決してから、固定したワークスペースの同期を行います。
4. 選択を公開する前に候補を検証します。候補が失敗すれば、前の環境が選ばれたまま残ります。
5. 今のプロセスが前の環境を使っている場合、PM は Hermes の再起動が必要だと知らせます。読み込み済みのライブラリをその場で差し替えることはしません。

配布されたソース、ロック、署名済みの中身は変わりません。追加のツールや
Python の環境は、元の成果物の外にある書き込み可能な場所を使います。
プラグインの依存関係は、その全体の環境を共有します。Python のサンドボックスとして切り離されているわけでは
ありません。互換性のある依存の依存は変わることがありますが、宣言された
制約と正確に固定した版は守られます。

| 管理のしくみ | 振る舞い |
|---|---|
| 宣言された追加指定 | このヘルパーが受け付けるのは、プロジェクトの追加指定の名前です。任意の pip のコマンドは受け付けません。取り除かれた `LAZY_DEPS` の機能名の登録表は使いません。 |
| 検証済みのツール | 管理下のツールのアーカイブは、版と SHA-256 のハッシュが `pm/lock.json` に載っています。 |
| 原子的な選択 | 新しい実行環境の選択を公開する前に、準備と検証を済ませます。 |
| 失敗の報告 | 失敗すると `pm.InstallError` を投げます。PM の同期の受領記録には、失敗した手順と方針による拒否が含まれます。 |
| プラグインを自動で外さない | 依存関係をまとめるのに失敗しても、入っているプラグインを黙って無効にしたり消したりしません。 |

その場での導入を止めるには、次を実行します。

```bash
hermes config set security.allow_lazy_installs false
```

すでに入っている依存関係はそのまま使えます。PM の導入コマンドを明示して実行するのは、
その場での導入とは別のものです。同梱版に固定の機能一覧がある場合、遅延導入を止めていると、
求められた Python の追加指定の名前がその一覧に絞られます。この設定は、プラグインを明示して受け入れることや、
手動のパッケージマネージャーのコマンドまで一律に禁じるものではありません。公式の Docker イメージも、
内部の環境方針によってその場での導入を止めています。

依存関係が足りないときは、`hermes tools` と `hermes doctor` で
何が必要かを確かめてください。署名済みの中身やシステムの Python に対して pip を実行しないでください。
導入を誰が受け持つか、診断、コマンドの境界については [パッケージ管理](/hermes/docs/reference/package-management/) を参照してください。
