...
upstream_blob: 8d2679c193283b32f62d4899593e3bff2162935f
sources:
...
macOS では、`F7` は物理的な F7 ファンクションキーを指します。MacBook のキーボードでは、この列がシステムの操作に割り当てられていることがあります。その場合は **Fn**（新しいキーボードでは **地球儀** キー）を押しながら **F7** を押すか、**システム設定 → キーボード → キーボードショートカット → ファンクションキー** で **F1、F2 などのキーを標準のファンクションキーとして使用** をオンにしてください。それでもターミナルがキーを渡してくれないときは、下にある同じ働きの **Ctrl+R** を使ってください。
...
- 継続中の **`/goal`** には、作業中の一覧の上に専用の行が出ます（`⊙ goal · 3/20 turns · …`、または理由付きの `⏳ goal parked` / `⏸ goal paused`）。ゴールが完了するか取り消されると、この行は消えます。順番待ちの追加指示はその上に並びます。
- **`Ctrl+R`** は、その一覧をふだんのプレビュー表示と1行の要約表示とで切り替えます。ターミナルがファンクションキーを渡してくれる環境では、**`F7`** も代わりのキーとして引き続き使えます。監視画面が開いたり入力欄からフォーカスが移ったりはしません。この選択は設定を変えずに、いま動いている TUI の中だけで有効です。
