...
- **返信の文脈** — 利用者が以前のメッセージに返信すると、エージェントはもとの文面も文脈として受け取ります。画像、ボイスメモ、動画、文書を引用すると（自分が送ったものでも、cron で届いたグラフのようにボットが送ったものでも）、そのファイルもそのターンに添えられるので、引用した画像の下に「これは何？」と書けば通じます。Meta の webhook が運ぶのは引用されたメッセージの ID だけなので、最近送受信したメッセージを記録した手元の索引（ゲートウェイごとに直近 1000 件）から引き当てます。それより古い引用には、ファイルが添えられません。

### 送る側 {#outbound}

- **文章** — マークダウンは WhatsApp 独自の書式に自動で置き換えられます（`**bold**` → `*bold*`、`~~strike~~` → `~strike~`、見出しは太字、`[link](url)` → `link (url)`）。長いメッセージは 4096 文字ごとに分けられます。
