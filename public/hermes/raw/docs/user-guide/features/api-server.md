...
細かい実況は、子ごとの実況の記録ファイルを見てください。これらのできごとは、親のストリームが
開いているあいだだけ流れます。離れて完了したものが遅れて届いても、終わった実行の SSE ストリームを
再び開いたり、その終了状態を変えたりはしません。

#### 切り離された結果とセッションの履歴 {#detached-results-and-session-history}

裏で動く委任は、サーバー側のセッションの履歴を読み込む続きの往復があってはじめて働きます。つまり
Chat Completions でのはっきりした `X-Hermes-Session-Id`、Hermes 本来の `/api/sessions/{id}/chat`
のリクエスト、あるいはセッションの履歴を使う Runs のリクエストのいずれかです。ヘッダーのない
Chat Completions、Responses の数珠つなぎ、そして `previous_response_id` や呼び出し側が渡した
履歴を使う Runs のリクエストは、代わりに委任を同期で実行し、もとの往復のなかで結果を返します。
リクエストの内容からセッション ID をただ導き出しただけでは、切り離した配達は働きません。

再開できるリクエストでは、委任のまとまりごとに完了が 1 回だけ保存されます。これは
`GET /api/sessions/{id}/messages` から取り出せますし、次に実際にクライアントが往復するときの
セッションの履歴にも入ります。やり直しても同じ結果が二重に入ることはなく、途中の失敗を知らせる
ものは別の身元を持ちます。配達は、クライアントの往復がセッションの占有権を持っているあいだは
待たされ、圧縮による続きにも従います。Chat Completions は、あなたが渡したはっきりしたセッション
ID を JSON でも逐次送りでもそのまま返すので、圧縮のあともその ID を送り続けてください。

完了が**求められていないモデルの往復を勝手に始めることはなく**、保留中の人による確認を素通りする
こともありません。次の往復はクライアント側のものです。自分で持っている履歴のスナップショットを
使い続けるクライアントは、サーバー側の配達の記録がそのスナップショットに混ざることを期待するので
はなく、同期の委任を使ってください。
