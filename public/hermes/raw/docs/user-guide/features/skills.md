...
- **作るのはローカル、書き換えはその場で**: エージェントが新しく作ったスキルは `~/.hermes/skills/` に書かれます。すでにあるスキルは見つかった場所で書き換えられ、これは `external_dirs` の下にあるスキルも同じです（エージェントが `skill_manage` の `patch`、`edit`、`write_file`、`remove_file`、`delete` といった操作を使ったとき）。
