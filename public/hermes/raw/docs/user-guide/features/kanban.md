...
### 上限に届く前の区切りの知らせ {#completion-checkpoints-before-the-iteration-cap}

割り振り役が抱える作業役には、決められた繰り返しの上限の 9 割あたりで、区切りの知らせが 1 度だけ
届きます。まだツールを呼べる回が残っているうちに、新しいツールの結果へ添えて渡されます。
`agent.budget_warning_ratio` を使えば、もっと手前で知らせるようにできます。上限がごく小さいときも、
最後からひとつ手前までには必ず知らせます。1 回しか回らない設定には、上限の手前の区切りがありません。
知らせは、次の要求を出す前にセッションの記録へ残ります。作業役は、仕事の取り決めを確かめてから
`kanban_complete` を呼ぶか、途中経過をコメントとして残して続けてください。コミットや差分だけで
仕事が自動的に完了になることはありません。

固い上限、ツールを使わない最後のまとめ、失敗が続いたときの遮断は、これまでどおりです。
それでも上限を使い切った作業役は、回数を区切った再試行の対象のままです。これは報告の機会であって、
モデルが知らせに従うことを約束するものではありません。ふつうの会話や、任せられた子は、
この自動の区切りを受け継ぎません。そちらの繰り返しの警告は、これまでどおり自分で有効にする形です。
...
8 つの代表的な協働の型は、この下の [協働の型](#collaboration-patterns) にまとめてあります。

## PR の完了の取り決め {#pr-completion-contracts}

作成のときに `--completion-contract OWNER/REPO` を付けて、PR を伴う仕事だとはっきりさせます（すでに
ある作業なら `https://github.com/OWNER/REPO/pull/123` のような正確な URL を渡します）。`kanban_create`
も同じ `completion_contract` を受け取ります。意図して手元だけで終える作業には `local-only` を使います。
すでにあるカードと、何も書いていないカードは、これが既定のままです。本文に書いた URL は取り決めに
なりません。

公開したあとは、完了のときに `metadata.published_pr` を渡します。最初に一致した URL がそのカードに
ずっと結び付き、やり直しのときに別の通っている PR で置き換えることはできません。CLI の `show --json`
と `kanban_show` で、保存された取り決めを見られます。

共通の `complete_task` の境目は、作業役のツール、CLI、レビューの承認、ダッシュボードからの完了の
すべてを覆います。ここでは、昔ながらのブランチ保護と、有効なルールセットが求める検査を読み、
先頭のコミットにぴったり一致する check run と昔ながらの status をページをまたいで集めたうえで、
PR の先頭と土台をもう一度読み直します。任意の検査が失敗していても飛ばされていても、通った**必須**の
検査を覆すことはありません。**必須**の証拠が、無い・待ち・失敗・取り消し・時間切れ・古い・飛ばされた・
どちらでもない、のどれかなら、そのカードは完了できません。1 度も走っていないのに受理する、決まりが
読めない、GitHub の API が失敗する、これらも同じです。必須の検査を持たないリポジトリには、手元だけの
取り決めが要ります。`gh` は、そのリポジトリの検査と決まりを読める形で認証されている必要があります。
この関門が遠くへ書き込むことはありません。

はねられても、動いているカードと作業場所はそのまま残ります。長く残る `pr_acceptance` の出来事に、
PR の URL、SHA、必須の検査、check の id と URL、分類、立て直しの手順が入ります。次の一手は
`last_failure_error` が示します。失敗を直すか、土台側の検査を回し直すか、少し待ってから、もう一度
完了を試してください。人の手が要るときは `kanban_block` を使います。GitHub のそっけない `failure`
だけでは、テストが落ちたのか成果物の受け渡しが落ちたのかは分かりません。残っている URL を見てください。
土台側の不調だとはっきり分かるものと、API の失敗は、別のものとして分けて扱われます。このために作業役が
増えることはありません。

受け取りの記録と、最後の書き込みでの実行・状態・取り決めの持ち主の確かめは、SQLite の同じ錠の中で
行われます。取り上げられた作業役が、新しい実行を完了させたり、そこへ受理を貼り付けたりすることは
できません。最後の GitHub の読み取りは、完了したその時点の写しであって、分散した取引でも、完了後も
ずっと見張る仕組みでもありません。これは 1 人で使うときの一生の見張りであって、データベースへ直に
書き込むあらゆるコードを OS のレベルで締め出すものではありません。GitHub Enterprise は対象外です。
公開と一生に関わる関連の作業は #91230、#84254、#52311 です。手元での検証と公開だけでは、遠くでの
受理にはなりません。
...
| `notify_in_gateway` | `true` | このゲートウェイで、かんばんの購読を見回って届けます。届け先の購読をひとつも持たないプロファイルでは `false` にすると、何もないのに 5 秒ごとに回る通知役の見回りが止まります。`dispatch_in_gateway` とは別ものです。差し向けをしないゲートウェイでも、プロファイル専用の届けのアダプターを持っていることがあります。 |
...
割り振り役の作業役が `kanban_create` や `hermes kanban create` で仕事を作ると、`parents` の
つながりが無くても、持ち主の仕事に付いている知らせの登録がそのまま引き継がれます。届け先、経路の
目印、届け方は保たれ、受け身の登録が自動で起こす形へ格上げされることはありません。これは、いまの
会話を新しい届け先として足すかどうかを決める `auto_subscribe_on_create` とは別に働きます。CLI だけの
セッションや、持ち主の仕事にひとつも登録が無い作業役に、届け先が勝手に作られることはありません。

`kanban_create` では、どのセッションの流れを継ぐかが次の順で決まります。はっきり渡された `session_id`、
持ち主である作業役の仕事が持つセッション、要求ごとの API の出どころ、そしていまのプロセスの
セッションです。組み込みの分解も、大元のセッションを受け継ぎます。セッションの流れそのものは
知らせの届け先ではありません。`session_id` を変えても、すでにある登録が置き換わることはないので、
届け先を変えるときは `notify-subscribe` と `notify-unsubscribe` を使ってください。
...
`notify+wake` では、受け身の知らせを送るだけでなく、起こす分が連携側の応答の列に受け付けられて
はじめて配信が終わったことになります。受け手が居ない、経路がはねられた、列がいっぱい、といった
ときは、登録を切らずに次の見回りでやり直します。送った知らせは SQLite に別々に記録されるので、
起こす分がはねられても、すでに記録された知らせが二重に送られることはありません。`notify` は受け身の
ままで、応答を始めることはありません。受け付けられたことは、モデルが動いた保証でも、うまく返せた
保証でもありません。ふつうの応答の関門はそのまま働きます。ちょうど 1 回の配信を約束するものでも
ありません。送信と記録のあいだにプロセスが落ちれば知らせが重なることがありますし、いまの
「配信の前に確保する」しくみは、異常終了から復旧できる列ではありません。
...
手引き](/hermes/docs/user-guide/features/kanban-multi-gateway/))
...
- **知らせを届けるのはプロファイルの持ち場です。** どのゲートウェイも——差し向けを
  ...
  `writer` のゲートウェイが届けます。
- **経路を振り分けるだけのプロファイル**は、登録に残っているプラットフォーム、チャット、
  スレッド、範囲、親チャンネルの目印が、`gateway.profile_routes` を通してそのプロファイル
  そのものに解けるなら、主となる口を使えます。つながっている二番目の口があれば、そちらが
  正です。二番目の口の登録が欠けているときに、主のボットへ落ちることはありません。一致しない、
  割り当てが変わった、無効になった、どちらとも取れる経路は、届かないまま残り、あとでやり直せます。
  必要な目印が欠けた古い行を、当て推量でどれかのプロファイルに結び付けることもしません。起こす
  ひと回りは、届け先のプロファイルの実行環境と、許された通り道をそのまま使います。
