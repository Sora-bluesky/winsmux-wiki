...
- **デスクトップ** — **返信を読み上げる**はデスクトップだけの設定で、設定 → 音声にあるゲートウェイ側の `voice.auto_tts` とは切り離されています。共有していた値を最初に一度だけ引き継ぎ、そのあとゲートウェイの設定を変えても、デスクトップの切り替えが上書きされることはありません。手元の保存領域がいっぱいだったり使えなかったりしても、選んだ状態はそのウィンドウのあいだは保たれます（読み込み直したあとまで残るかどうかは、できる範囲での対応です）。**返信を読み上げる**を有効にしたり、**音声での会話**を始めたりすると、その場で設定済みのエンジンを裏で先に読み込みます。両方とも切ると、常駐していたモデルを解放します（Piper の声は数十 MB、KittenTTS は最大 80MB ほどあるので、無駄にメモリに置いたままにしません）。
- **CLI / TUI** — `/voice tts`（および `voice.auto_tts` を設定してあるときの `/voice on`）が同じことをします。`/voice off` で解放されます。

切り替えのそれぞれがエンジンの*使用権*を持ちます。モデルが解放されるのは、あらゆる画面にまたがる最後の使用権が手放されたときだけです。だから、あるデスクトップの窓で読み上げを切っても、別の窓で進んでいる会話から声を取り上げてしまうことはありません。解放は、最後の使用権が手放されてから `tts.keep_warm_seconds`（既定は `60`）秒待ってから行われます。その間にどこかの切り替えで音声出力がふたたび有効になれば、読み込んだモデルはそのまま残ります。そのため、ウェイクワードの待ち受けを繰り返すときや、音声での会話をすぐにやり直すときに、毎回声を読み込み直さずに済みます。`0` にするとすぐに解放されます。クラウドの提供元には抱えておくモデルがないので、切り替えは必要になってから入る SDK（edge-tts、ElevenLabs、Mistral）があるかを確かめるだけです。先読みはできる範囲での話で、エンジンが読み込めなくても切り替え自体は成功し、最初の返信はこれまでどおりその場での読み込みに落ちます。
