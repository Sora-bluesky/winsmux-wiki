...
upstream_blob: 0c03f4e8cb897bbfd0af29375d7a245fa5751499
sources:
...
### ローカルモデル: 振り返りは GPU が空くのを待つ（`defer`） {#local-models-reviews-wait-for-an-idle-gpu-defer}

クラウドのプロバイダーでは、振り返りは数秒で終わり、次にあなたがすることと並んで走ります。振り返りの実行先が **管理下のローカル llama-server**（設定 → Local models）のときは、同じ枝分かれが、次のプロンプトに要る GPU を占有します。大きなモデルでは数分かかることもあり、新しいプロンプトを送るとそれを打ち切ってしまい、学びが失われます。そのため、管理下のローカルの実行環境では、振り返りは **既定で先送りされます**。ターンの終わりにキューへ積まれ、機械がしばらく静かになった落ち着きの時間を過ぎてから実行されます。振り返りそのものは何も変わりません — 同じモデル、同じ会話全文のたどり直し、同じ書き込み — 変わるのは実行される瞬間だけです。
...
| 値 | 振る舞い |
|-------|-----------|
| `auto`（既定） | 実行先が管理下のローカルサーバーに決まる振り返りはキューへ積まれ、機械が空いたときに走ります。それ以外の実行環境（クラウド、外部サーバー）は、これまでどおりすぐに生まれます。 |
| `never` | どこでも従来どおりの振る舞いです。管理下のローカル GPU 上でも、ターンの終わりにすぐ生まれます。 |

キューに積まれた振り返りは、セッションごとにまとめられます（新しいターンの写しが古いものを置き換えますが、振り返りは会話全体をたどり直すので何も失われません）。新しいプロンプトに割り込まれた振り返りは、捨てられるのではなくキューへ戻され、`defer_max_age_s` より長く待った振り返りは、機械が一度も空かなくても実行されます。はっきりと `/refine` を呼んだときは、常にすぐ実行されます。キューはメモリ上にあるだけなので、アプリを終了した時点でまだ待っている振り返りは、実行中の枝分かれと同じように失われます。
