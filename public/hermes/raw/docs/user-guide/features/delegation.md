...
upstream_blob: 90c5b9fe162d75057a8767147afdfed3bb088c4b
sources:
...
いちばん上のエージェントが `tasks` の配列を渡すと、Hermes は裏で走る受け取りの控えをひとつ返し、サブエージェントを並行で走らせます。結果は最後にまとめて返るのではなく、**まとまりが終わるたび**に返ります。

- `group` を書かない作業は、それだけでひとつのまとまりです。そのサブエージェントが終わった時点で結果が会話へ戻るので、独立した 5 件の PR の見直しは 5 通のメッセージとして届き、エージェントはいちばん遅いものを待たずにそれぞれへ手を打てます。
- 同じ `group` の文字列を持つ作業は互いを待ち、**ひとつ**にまとめたメッセージとして返ります（親が結果を比べたり合わせたりしなければならないときに使います）。
...
受け取りの控えには、まとまりごとの情報（`units[].delegation_id`、`group`、`task_indexes`）が並びます。まとまりの ID は呼び出しの ID に `-1`、`-2`、… を付けたものです。ひとつの呼び出しのすべてのまとまりは `delegation.max_concurrent_children` の枠をひとつだけ使うので、まとめ方によって使える量が変わることはありません。取りまとめ役のサブエージェントは、結果をまとめられるように、いまの応答の中で自分の一括分をすべて待ちます。
...
- **進み具合の表示:** CLI では、各サブエージェントのツール呼び出しが木の形でその場で見え、作業ごとの完了の行も出ます。プラットフォーム連携では、進み具合はまとめられて親の進捗の受け口へ渡されます
