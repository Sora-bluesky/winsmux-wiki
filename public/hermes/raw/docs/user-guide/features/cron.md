...
cron ジョブは、プロンプトを走らせる前にスキルをひとつ以上読み込めます。それぞれのスキルは、チャットのセッションで `/skill-name` から読み込むときとまったく同じように読み込まれます。`config.yaml` から解決した `metadata.hermes.config` の値を載せた `[Skill config ...]` のブロックも含みます。
...
`hermes pause [--reason ...]` は、全体をまとめて止める非常停止です（`hermes resume` で解除します）。これがかかっている間は、どの入口から来ても、予約された cron の発火は始まりません。組み込みのティッカーは振り分けを飛ばします。managed-cron（ホスト型のスケジューラ）の発火用 webhook は `503` と `Retry-After: 60` を返すので、スケジューラは再開したあとに発火を届け直します。[取りこぼした実行の追いつき](#misfire-catch-up) の見回りも、止めていた分をまとめて強制的に発火させることはせず、じっとしています。すでに走っている実行が止められることはなく、失われるものもありません。期限の来た作業は、`hermes resume` のあとの最初のティックか見回りで追いつきます。明示的な手動実行（`hermes cron run`、ダッシュボードの Trigger ボタン）は運用者による上書きなので、一時停止中でも実行されます。
...
- **ジョブは自分自身を消しても報告できる。** 「X を見張って、一度だけ知らせて、それで終わる」
  という型、つまり実行の中で `cronjob(action="remove", job_id=<its own id>)` を呼んでから
  答える繰り返しのジョブは、その最後の応答を届け、実行を `completed` として記録します。
  そのあとジョブの記録と `cron/output/<job_id>/` のディレクトリは消えていて、最後の実行は
  そこには書かれません。実行の*外*から記録を消した場合（別のプロセスや、同じ ID を使い回す
  置き換えのジョブ）は、これまでどおり古い実行の出力は捨てられます。
...
- 本来のチャットが、メールボックスに対応した Desktop / TUI のバックエンドで開かれている場合、ボットが空いていても取り込み中でも、配信は**その場で確実に待ち行列へ入ります**。受信のターンを走らせるのは、そこにいる持ち主だけです。cron が競合する CLI の書き手を立ち上げることはありません。CLI だけの持ち主や、対応していない古い持ち主がチャットを押さえている場合、cron はまだ始まっていない出力を、送る側のプロファイルの `cron/bot_chat_pending/<receipt-id>.json` に取っておきます。その持ち主がチャットを手放したあと、以降のスケジューラのティックが、受け付けた順に届けます。後回しになった作業は、スケジューラの起動元のルートが変わっても、受け付けたときの宛先のホームと受領 ID を保ちます。宛先が見つからない・名前が変わっているときに、作り直したり別のプロファイルへ解決したりはしません。`transferred` の保留記録は、そこにいる持ち主の受領を指すもので、失敗したターンではありません。壊れた JSON の記録は残したままログに出し、ほかの待ち行列の出力を妨げません。持ち主がいなければ、これまでどおり `hermes chat -c "Bot Chat" --create-if-missing` の経路が使えます（通常のセッション所有権のチェックはそのまま働きます）。その子プロセスは、独自のルートも含め、cron がすでに確かめた宛先のホームそのものを使います。受け継いだ `HOME` や、切り替わった現在のプロファイルがその向き先を変えることはできません。宛先のディレクトリが無ければ、作り直さずに起動の前に断ります。後回しの依頼は、その経路を起動する前に取り出し済みとして確保されます。途中で止まっても、サブプロセスの結果がはっきりしなくても、自動で送り直すことはありません。
- まだ始まっていない出力に有効期限はありません。対応していない持ち主がずっと手放さなくても、黙って捨てられることはなく、待ち行列に残り続けます。受領はその中身をいつまでも保ちます。配信中の予期しない例外はログに出して `ambiguous` として残し、同じ吐き出しの中のほかの配信は止めません。取り出し済み・あいまいな試みが自動でやり直されることはありません。
