...
upstream_blob: bd929fc094590d81c0c3e9c34c57ae14cf24eda3
sources:
...
- **DM しかないプラットフォーム**（WhatsApp、Signal、SMS）にはスレッドが無いので、まとめは
  作成元の DM のセッションへ写されます。DM そのものが、続きを話す場になります。
...
一斉配信・拡散の宛先（`all`、プラットフォーム名だけのホームチャンネル）が続けられるように
なることはありません。写しはラベル付きの利用者のターン（`[Cron delivery: <task name>]`）
