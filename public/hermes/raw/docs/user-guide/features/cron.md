...
- プロバイダーの API キーが解決できること（固定していないジョブで `fallback_providers` の連鎖が
  設定されているときは省きます。主キーが欠けていてもフォールバック経路が救うことがあるためです。
  固定したジョブはその連鎖を使わないので、常に検査します）、
...
ジョブを預かったことを伝えます。**間隔のまばらな** cron ジョブで、次の本来の予定よりずっと前
（スケジュールの周期の半分以上早く）にプロバイダーの枠が開き直した場合は、止められていた予定の実行が
その回復の時点で 1 回だけ再試行されます。そこでもう一度枠の不足で失敗したら、本来の予定を待ちます。
間隔の詰まったスケジュール、手動の実行、一定間隔のジョブは、本来の次の実行時刻をそのまま保ちます。
それ以外の場合は、逃した予定の回がひとつにまとめられ、
`next_run_at` は枠が明けたあとの最初の予定時刻へ動きます。預かりの解ける時刻は
`quota_hold_until` として保存され、それまでは何も起動せず、アラートも出ません。モデルまで届いた実行が
...
主となる API キーがレート制限に掛かったり、プロバイダーがエラーを返したりしたとき、cron のエージェントは次のことができます。

- 同じプロバイダーの[認証情報プール](/hermes/docs/user-guide/configuration/#credential-pool-strategies)にある**次の認証情報へ回す**。これは固定の有無にかかわらず、すべてのジョブに当てはまります。
- `config.yaml` の `fallback_providers`（または旧来の `fallback_model`）にある**別のプロバイダーへ切り替える**。ただし**固定していないジョブだけ**です。実行が始まる前の認証情報の解決に失敗した場合と、実行の途中でプロバイダーがエラーを返した場合の両方が対象です。

自分の `provider`・`model`・`base_url` を持つジョブ（`--provider` / `--model`、`--pin`、ダッシュボード、`jobs.json` で設定したもの）は、グローバルの連鎖へ切り替わることはありません。固定は、そのジョブがどの経路で動くかを示すものです。フォールバックの項目は別のプロバイダーで、たいていは別のモデルでもあります。そのため、固定した経路が失敗すれば実行は失敗し、失敗の通知にもそう書かれます。これは[サブエージェントへの委任](/hermes/docs/user-guide/features/delegation/)が、固定した子に当てはめているのと同じ規則です。あるジョブでフォールバックを使い続けたいなら、固定しないでおきます。そうすれば `cron.model` / `cron.model_provider`（またはメインのモデル）に従い、ほかの固定していないジョブと同じように連鎖をたどります。

この規則ができる前は、プロバイダーが失敗した固定ジョブが、代わりに `fallback_providers` のうち最初に動く項目で実行されることがあり、出力に 1 行の通知が残るだけでした。その動きを頼りにしていた場合は、ジョブの固定を外し（`hermes cron edit <job_id> --unpin`）、代わりに `cron.model` でモデルを設定してください。

つまり、同じプロバイダーの別の認証情報があれば、キーひとつがレート制限に掛かっても実行は失敗しません。また、連鎖を設定していれば、固定していないジョブはプロバイダーの障害が起きても動き続けます。
