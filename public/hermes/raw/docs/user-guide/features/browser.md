...
:::note 動いているブラウザは、ログインや自動入力のデータベースを塞ぐことがあります
...
macOS と Linux では、プロファイルにファイルの錠はかかりません。ただし認証のデータベースはどれも SQLite のオンラインバックアップで写しを取る仕組みで、持ち時間は5秒です。そして動いている Chrome はたいてい `Login Data`、`Login Data For Account`、`Web Data` を書き込みの錠で握り続け、その持ち時間のあいだに手放しません（`Cookies` はふつう問題なく写せます）。そうなると Hermes は起動をやめ、読めなかったデータベースの名前を挙げます。たとえば「chrome is running and holds the profile's Login Data, Login Data For Account, Web Data with a write lock」といった具合です。ですから実際のところ、**macOS や Linux でも、本物のプロファイルのセッションを起動する前にブラウザを終了してください**。セッションが立ち上がったあとなら開き直してかまいませんが（写しは別のディレクトリにあります）、認証のファイルは新しいセッションを起動するたびに同期し直すので、開いたままのブラウザは次にまた同じ錠にぶつかります。Hermes は生のファイルのコピーに落ちる代わりに SQLite を通して確定済みの WAL のデータを守るので、最近のログインを黙って失うことがありません。読めないデータベースや壊れたデータベースがあるときも起動はそこで止まり、メッセージには SQLite のエラーが添えられます。

`browser.real_profile_autoclose: true` を設定すると、プロファイルの錠を握っているブラウザ（Windows の場合です）を **Hermes が閉じましょうかと申し出る**ようになります。これを入れていても、Hermes が勝手に閉じることは決してありません。プロファイルに錠がかかっていれば必ず止まり、エージェントがまずこちらに尋ねます。こちらが承認したときにだけ `hermes browser close-profile` を実行し（そのプロファイルにつながるブラウザのプロセスの一群を終わらせます。保存していないタブは失われます）、そのうえでやり直します。そのあともまだ錠がかかっていれば（たとえば裏や通知領域のものが再び立ち上がったなら）、Hermes は止まったまま、ブラウザを完全に終了するよう伝えます。自分で繰り返したり、もう一度終わらせに行ったりはしません。
:::
...
#### 定期実行と無人での実行 {#scheduled-and-unattended-runs}

実際のプロファイルを使うセッションなら、すでにサインインしているサイトを `cronjob` から操作させられます。
スナップショットは既定で画面を出さずに（ヘッドレスで）動きます。認証ファイル（`Cookies`、
`Login Data` など）は、新しいセッションを始めるたびに実際のプロファイルから同期し直します。
すでに開いているセッションはそのまま使い回すので、同期が起きるのは新しいセッションを起動したときです。保管庫に認証情報を保存していない場合、そのサイトのセッションが切れていると
ログインページが表示されます。無人の定期実行はそこで止まったまま待たず、その状態を報告します。

定期実行を組む前に、次の 3 つを済ませてください。

- **切り替えをオンにします。** `browser.use_real_profile` の既定は `false` で、
  オフのままだとジョブはサインインしていないまっさらなプロファイルで動きます（上の説明を参照）。
- **ジョブにブラウザのツールセットを渡します:**
  `cronjob(action="create", enabled_toolsets=["browser", ...], ...)`。ジョブごとの
  指定は、cron プラットフォームの設定より優先されます
  （[詳しく](/hermes/docs/user-guide/features/cron/#toolsets-available-to-cron-jobs)）。
- **途中で何も入力できない前提にします。** cron、webhook、API、
  `hermes chat -q` のセッションでは入力に答える人がいません。そのため、同期した Cookie だけでは
  使えないサイト（ログインフォームや、新たに求められる 2 段階認証）では、認証アプリの鍵も含めて認証情報を前もって保存しておく必要があります。これは
  [認証情報の保管庫](/hermes/docs/user-guide/features/credential-vault/#headless-sessions)の役目で、
  自分のセッションが切れたあとの実行を支えるのもこの保管庫です。

**Windows での前提条件:** ブラウザを完全に終了していないとスナップショット自体が取れません。
そのため、定期実行の前にブラウザを閉じておく必要があります。Hermes が確認なしにブラウザを閉じることはありません（上の注記を参照）。

同梱の `product-price-monitor` スキルは、繰り返し実行する形の実例です。
セットアップ時に JSON で監視の取り決めを書き、定期実行のたびにそれを読み直して比べ、変化があれば知らせます。
ただし、この例が扱うのは定期実行の部分で、ログインは扱いません。
