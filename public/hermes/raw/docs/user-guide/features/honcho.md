...
ディレクトリに基づく方式と、手で書いた `sessions` の対応付けは、バックエンドを起動したディレクトリではなく、セッションの論理上の作業ディレクトリを使います。Desktop / TUI のプロジェクトの作業場所や ACP のセッションのディレクトリは、エージェントを組み立てるとき（後回しにした組み立ても含みます）に渡されます。ディレクトリが渡されない場合、Honcho は実行時の cwd の決め方に従います。まずセッションの文脈、次にスコープ付きの `terminal.cwd` の設定、最後にプロセスを起動したディレクトリです。

メッセージングのゲートウェイは、方式やタイトルに関係なく、チャットごとの固定のセッションキーを使い続けます。それ以外のセッションでは、`per-session` による識別が最優先で、次に手で書いたディレクトリの対応付け、明示的なタイトル、設定した方式の順になります。

Hermes が自動で付けたタイトル（`derived` または `llm`）は、Desktop の枝分かれに付く系譜のタイトルも含めて表示のための情報にすぎず、`sessionStrategy` を上書きしません。利用者が明示的に付けたタイトルは、ゲートウェイでも `per-session` でもなく、手の対応付けも無いセッションでは、意図してセッション名を上書きするものとして扱われます。

タイトルの出どころを記録するようになる前に作られたセッションは、以前の挙動のままです。古い自動のタイトルと古い利用者のタイトルを見分けられないため、出どころの無いタイトルは明示的な上書きとして扱われます。
