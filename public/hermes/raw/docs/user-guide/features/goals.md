...
3. **作業場所が変わっていなければ、走らせ直しません。** 関門が落ちたあと、作業場所が何も変わっていなければ（HEAD と作業ツリーの状態から取る git の指紋で見ています）、関門は走らせ直さず、記録してある失敗をそのまま出して試行回数だけ進めます。行き詰まったエージェントが、同じ赤いテストを走らせ直して時間を使い切ることはありません。git のリポジトリの外では、関門は毎回走ります。
...
| コマンド | 何をするか |
|---|---|
| `/goal wait <pid> [reason]` | その PID のプロセスが終わるまで、手動で待たせます。 |
