...
| `Ctrl+T` / `F6` | 入力欄の下書きを失わずに、サブエージェントの様子を全画面で追う画面を開きます。実況用の枠はステータスバーの上に自動で現れます。矢印キーでワーカーを選び、`Enter` で直近のログを表示、`s` で指示を足し、`x` で確認を挟んだうえで停止を頼めます。[サブエージェントの様子を見る](/hermes/docs/user-guide/features/delegation/#monitoring-running-subagents-agents) を参照してください。 |
| `F7` | サブエージェントの実況枠の表示を、複数行のプレビューと1行の要約の間で切り替えます。入力欄のカーソルは動きません。 |
| `Ctrl+D` | 終了します |
