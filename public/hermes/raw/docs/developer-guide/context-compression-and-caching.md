...
## Bedrock のコンテキストウィンドウのキャッシュ {#bedrock-context-window-cache}

`agent/model_metadata.py` での Bedrock のコンテキスト解決は、次の優先順位に従います。

- **明示的な上書きが勝ちます。** 設定されたコンテキスト長は、キャッシュ・プローブ・静的な表よりも優先されます。
- **プロバイダーが確認した上限は残り続けます。** プローブの成功や、プロバイダーのエラーから学習した上限は、静的な表より小さい値であっても正式なものとして扱われます。圧縮側も再起動後に同じ値を使います。
- **古いエントリは検証し直します。** 以前の単一値のエントリには出どころの記録がなく、プローブの結果かフォールバック値かのどちらでもありえます。値の大きさからはどちらかを判別できません。
- **プローブに失敗したときは、その時点の表を使い、保存はしません。** 失敗にはメモリ上で5分のクールダウンがあり、Hermes home・エンドポイント・モデル・リージョンの単位で分かれています。期限が切れるか、キャッシュを明示的に無効化すれば、もう一度試せます。
...
予算を使い切った時点で止めます。この予算と、行をまるごと含めるときに予算を超えてよい 1.5 倍の
ゆるい上限は、どのモデルでもコンテキストウィンドウの 20% までに抑えられます。そのため `protect_last_n` が
*最小数*として働くのは小さな件数の下限（8 行）までで、圧縮の余地を残せないほどの末尾を強制することは
ありません。この上限を超えてよいのは、必須のアンカーである最後の利用者メッセージと最後のアシスタント
メッセージ、そして分割できないツールの組だけです。
...
`"auto"` は、`agent/agent_init.py::_init_prompt_cache_config` の中で `agent/prompt_caching.py::auto_cache_ttl_for_source` を通じてセッションごとに 1 回だけ決まります。人が操作するペースの呼び出し元では `1h`、`MACHINE_PACED_SOURCES`（subagent、cron、oneshot、webhook、kanban、api、tool、batch）では `5m` になります。補助の呼び出しやスタブの呼び出し（`configured_cache_ttl()`）は、`auto` を `5m` として扱います。
