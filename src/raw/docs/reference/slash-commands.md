...
インストール済みのスキルも、どちらの入り口でも動的なスラッシュコマンドとして現れます。（`/plan` はかつてこの一種でしたが、今は組み込みコマンドです。後述のセッションの表を見てください。）名前が組み込みコマンド（またはその別名）と重なるスキルには、`/<name>` は割り当てられません。組み込みの側が勝ち、スキルは `/skill <name>` で読み込めるままになります。`/skills list`、`/help skills`、コマンドパレットでは、そうしたスキルに `slash command /<name> unavailable — name taken by built-in; use /skill <name>` という印が付きます。
...
| `/queue <prompt>`（別名: `/q`） | 次のターンに送るプロンプトを予約します（今動いているエージェントの返答は中断しません）。CLI では、引数なしの `/queue` で予約中のプロンプトの一覧が出ます。送られる前に、`/queue list`、`/queue edit N <new prompt>`、`/queue rm N`、`/queue move A B`、`/queue clear` で中身を確認・整理できます。管理用の語で始まるプロンプトは、`/queue add <prompt>` で強制的に予約できます。 |
...
| `/loop [interval] <prompt> [--times N] [--until <condition>]`（別名: `/proactive`） | **このセッション**で、プロンプト（または別のスラッシュコマンド）を一定の間隔で繰り返し実行します。決まった間隔で回す（`/loop 5m check the deploy`）ほか、間隔を指定しなければ続けて何度も実行します。`--times N` で実行回数の上限を決め、`--until <condition>` を付けると、条件が満たされた時点で補助の判定モデルがループを止めます。サブコマンドは `/loop status`、`/loop pause`、`/loop resume`、`/loop stop` です。[ループ](/hermes/docs/user-guide/features/loops/) も見てください。 |
...
| `/resume [name]` | 名前を付けて保存したセッションを再開します。`/resume` だけを打つと、このチャットで名前を付けたセッションが、最後に動いた順に並んで出てきます（圧縮で何度も入れ替わった長い会話も 1 つの項目として、いま生きている先頭の状態で表示されます）。従来型の CLI では、ターンの実行中はこのコマンド（および `/sessions <id>`）は断られます。CLI は複数のセッションで 1 つのエージェントを共有しているため、ターンの途中で切り替えると、実行中のターンの残りが別のセッションの側に記録されてしまうからです。 |
...
| `/context [all]`（別名: `/ctx`） | コンテキストウィンドウの内訳を目に見える形で出します。CLI と TUI では 5×20 のブロックのマス目（1 マスがモデルのウィンドウの約 1% にあたります）と、カテゴリごとの推定値の表（システムプロンプト、ツール定義、ルール、スキルの索引、MCP、サブエージェント、記憶、会話）を空き容量と並べて表示します。メッセージングプラットフォームでは、自動圧縮のしきい値と余裕を示すゲージ、圧縮の統計、累積のやり取り量、そして同じカテゴリ表をテキストで出します。CLI、メッセージングプラットフォーム、計算ホスト上の TUI やデスクトップのセッションでは、出力の最後にファイルごとの **Context files** の一覧（.hermes.md、AGENTS.md の連なり、CLAUDE.md、.cursorrules と .cursor/rules/*.mdc、SOUL.md）が付きます。各ファイルの推定トークン数に加えて、読み込まれたか、`context_file_max_chars` を超えて切り詰められたか、優先度の高い別の種類のコンテキストに隠されたか、注入の検査で遮られたか、空または読めなかったか、インストール先のツリーを守る仕組みで外されたかがわかります。「CLAUDE.md が無視されるのはなぜ?」への答えがここにあります。`/context all` を使うと、スキルごと・ツールセットごとのコスト一覧（索引のコストと SKILL.md 読み込みのコスト、ツールセットごとのスキーマのトークン数）が加わります。読み取り専用でローカルで計算するので、LLM の呼び出しもプロンプトキャッシュへの影響もありません。 |
...
| `/loop [interval] <prompt> [--times N] [--until <condition>]`（別名: `/proactive`） | このセッションで、プロンプトを一定の間隔で繰り返し実行します。サブコマンドは `status`、`pause`、`resume`、`stop` です。[ループ](/hermes/docs/user-guide/features/loops/) も見てください。 |
...
- `/status`、`/egress`、`/version`、`/whoami`、`/bg`、`/btw`、`/queue`、`/steer`、`/voice`、`/reload-mcp`、`/reload-skills`、`/rollback`、`/diff`、`/debug`、`/fast`、`/approvals`、`/busy`、`/footer`、`/curator`、`/kanban`、`/topup`、`/login`、`/suggestions`、`/blueprint`、`/learn`、`/init`、`/sessions`、`/loop`、`/yolo` は、CLI とメッセージングゲートウェイの**どちらでも**動きます。
