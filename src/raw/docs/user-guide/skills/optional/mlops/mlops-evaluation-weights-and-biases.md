...
## この skill の使いどころ {#when-to-use-this-skill}

Weights & Biases（W&B）は、次のようなことをしたいときに使います。
- 指標を自動で記録しながら **ML の実験を追う**
- リアルタイムのダッシュボードで**学習の様子を見る**
- ハイパーパラメータや構成をまたいで **run を比べる**
- 自動の sweep で**ハイパーパラメータを詰める**
- バージョンと来歴を持たせて**モデルレジストリを管理する**
- チームのワークスペースで **ML のプロジェクトを共同で進める**
- データセット・モデル・コードといった**アーティファクトを来歴付きで追う**

**利用者**: ML の実務者 20 万人以上 | **GitHub のスター**: 10.5k 以上 | **連携先**: 100 以上
...
### 実験管理の基本 {#basic-experiment-tracking}
...
### 1. プロジェクトと run {#1-projects-and-runs}
...
データセット・モデル・その他のファイルを、来歴付きで追いかけます。

### アーティファクトを記録する {#log-artifacts}
...
### 2. 関係するものはすべて記録する {#2-log-everything-relevant}
...
### 3. 中身の分かる名前を付ける {#3-use-descriptive-names}
...
### 4. 大事なアーティファクトを保存する {#4-save-important-artifacts}
...
### 5. 接続が不安定なときはオフラインモードを使う {#5-use-offline-mode-for-unstable-connections}
...
### チームのプロジェクト {#team-projects}

- wandb.ai でチーム用のアカウントを作ります
- メンバーを追加します
- プロジェクトの公開範囲を決めます（非公開 / 公開）
- チーム単位のアーティファクトとモデルレジストリを使います

## 料金 {#pricing}

- **無料**: 公開プロジェクトは無制限、ストレージ 100GB
- **アカデミック**: 学生と研究者は無料
- **チーム**: 1 席あたり月 50 ドル、非公開プロジェクト、ストレージ無制限
