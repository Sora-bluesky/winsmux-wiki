...
upstream_blob: b792b962aa5d7a3b35e53d24294f12bb2e73253e
sources:
...
## デスクトップとダッシュボードでのメッセージングの状態 {#messaging-status-in-desktop-and-the-dashboard}

メッセージングの状態は、選んでいるマシンの、選んでいるプロファイルのものです。
`hermes gateway setup` で保存した資格情報があれば、`config.yaml` に `platforms` の
項目がなくても、資格情報で動くプラットフォームを有効にできます。`platforms.<name>.enabled: false`
とはっきり書いてあれば、それは無効のままです。ほかのプロファイルが、サーバーの
プロセスが持っている資格情報を引き継ぐことはありません。必要な資格情報の項目を持たない
プラットフォームが、その一覧が空だからというだけで有効になることもありません。

サーバー自身のプロファイルを名指ししたとき（たとえば既定のプロファイルで動いている
サーバーに `profile=default` を指定したとき）は、何も指定しないときと同じ状態が返ります。
**保存済み**は資格情報が保存されているという意味であって、メッセージングのゲートウェイが
動いているとか、プラットフォームにつながっているという意味ではありません。有効な
プラットフォームが**メッセージングのゲートウェイは停止中**と出ていても、それで正しいのです。
...
- 最後の送信が**流量制限**（Telegram のレート制限など）で断られたときは、記録された
  待ち時間が明けたあとに自動で送り直します。つなぎ直しも再起動も要りません。
  待っているあいだに再起動しても、保存してあった返答をそのまま引き継ぐので、
  送り直しの回数を減らすことも、エージェントを走らせ直すこともありません。送り直しでは、
  元のボットのプロファイル、チャット、スレッドがそのまま保たれます。流量制限からの
  復帰の前置きが、先のいくつかの断片はすでに届いているかもしれないと知らせます。
  台帳は、メッセージの長さから途中まで届いたかどうかを読み取ることはできません。
...
ゲートウェイでの会話は、何もしないまま時間がたっても、日付が変わっても、やり直されません。
新しい会話をはっきり始めたいときは `/new` か `/reset` を使ってください。文脈の圧縮は
これまでどおり自動で働きます。以前の `session_reset` の設定、やり直しの方針の上書き、
やり直しの時間を決める環境変数は、どれも無視されます。控えに置いてあるエージェントは
資源を取り戻すために手放されることがありますが、消えない会話そのものが差し替わるわけでは
ありません。再起動からの復帰でどこまでを新しいものとして扱うかは、自動で続きに入るかどうかを
決めるだけで、こちらがメッセージを送ったときに読み込まれる履歴を狭めるものではありません。
...
ゲートウェイからの脇からの差し込み（はっきり `/steer` と書いたものも含みます）と、動いている番への
行き先の切り替えは、きっかけになった出来事から分かる範囲のプラットフォーム、チャット、スレッド、
送り主、メッセージ、プロファイル、適用範囲の識別子を、メッセージごとの JSON の付帯情報として
持っていきます。`privacy.redact_pii: true` にしてあると、モデルから見えるこの付帯情報の識別子は、
対応しているプラットフォームでは伏せられた値になります。別名の識別子や親の識別子も同じです。
元の出来事の識別子は、宛先を決めるために内側に残ります。そうでなければ識別子はそのまま保たれます。
どちらの場合も、セッションのシステムプロンプトが変わることはなく、返信の代わりの宛先が
選ばれることもありません。この付帯情報は宛先を決めるための材料であって、権限を与えるものでも、
自動で届くことを約束するものでもありません。
