...
upstream_blob: 5af8fc9e07f96b8a21f6bd6fc1b5851381df10f4
sources:
...
| トークン数 | 使用中のコンテキストトークン数／コンテキスト上限。`~` は見積もりであることを表します |
| コンテキストバー | 色分けされたしきい値付きの、使用量を示すバー |
| 費用 | このセッションの推定費用（価格が不明またはゼロのモデルでは `n/a`）。料金は、まず Hermes に同梱された公式の価格表から、次にプロバイダーの `/models` の一覧から取ります。提供元直営の API（OpenAI、xAI、Anthropic、Google、DeepSeek、Xiaomi）に直接つないでいる場合は、どちらにも載っていないモデルでも models.dev にある提供元の定価で計算します。同じモデル ID を提供するプロキシ、リレー、カスタムのエンドポイントはその価格を引き継がず、`n/a` のままです。 |
...
コンテキストの数値や割合の前に付く `~` は、その値に手元での見積もりが混ざっていることを表します。
これはゲートウェイの `/status` と `/context`、TUI、デスクトップのコンテキスト計にも当てはまります。
プロバイダー側の利用状況がそのまま変わっていない場合に `~` は付きませんが、プロバイダーの値を
起点にして、まだ値付けされていない新しいメッセージが乗っている場合には付きます。どちらの値を
使ったかは `/context` が知らせます。分類ごとの内訳、空き容量、スキル、ツールセットの内訳は、
全体の占有率がプロバイダーの利用状況から来ている場合でも、つねに手元での見積もりです。
これらの表示上の目印が、圧縮の判断を変えたり、プロバイダーへの問い合わせを増やしたりすることはありません。
...
macOS では、`F6`/`F7` は物理的なファンクションキーのことで、最上段に表示されているメディアやシステムの操作のことではありません。ファンクションキーを押すときに **Fn**（新しいキーボードでは **地球儀** のキー）を押したままにするか、**システム設定 → キーボード → キーボードショートカット → ファンクションキー** で **F1、F2などのキーを標準のファンクションキーとして使用** を有効にしてください。ターミナルで確実に使える代わりのキーは、`F6` なら **Ctrl+T**、`F7` なら **Ctrl+R** です。
...
| `Ctrl+T` / `F6` | 入力欄の下書きを失わずに、いま動いている作業（サブエージェントと裏で走っているプロセス）を全画面で追う画面を開きます。実況用の枠はステータスバーの上に自動で現れます。矢印キーでワーカーやプロセスを選び、`Enter` で直近のログを表示、`s` でワーカーに指示を足し、`x` で確認を挟んだうえで停止を頼めます。[サブエージェントの様子を見る](/hermes/docs/user-guide/features/delegation/#monitoring-running-subagents-agents) を参照してください。 |
| `Ctrl+R` / `F7` | いま動いている作業の実況枠の表示を、複数行のプレビューと1行の要約の間で切り替えます。入力欄のカーソルは動きません。macOS がファンクションキーの列を占有しているときは、`Ctrl+R` が確実に使える代わりのキーです。実況枠には、サブエージェントと裏で走っているプロセスのほか、上の行に継続中の `/goal`（動作中、保留中、一時停止中のいずれかと、使ったターン数）、下の行に `/queue` で待っているプロンプトが表示されます。 |
| `Ctrl+D` | 終了します |
...
- **手元の端末で、セッションの作業ディレクトリで動きます。** 既定のローカルのターミナル
  バックエンドであれば、`!pwd` はエージェントから見えるものと一致します。リモートや隔離された
  `terminal.backend`（`ssh`、`docker` など）は `!` のコマンドには**使われません**。常に Hermes 自身が
  動いているホスト上で実行されるので、`!hostname` は手元の端末の名前を返し、エージェントの
  `terminal` ツールはバックエンドの名前を返します。バックエンドの*内側*で何かを実行したいときは、
  エージェントに頼むか、対象のシェルを開いてください。一方、入力欄のパス補完は設定した
  バックエンドに従い、対象側のファイル一覧を表示します。
...
Kitty のキーボードプロトコルを使うターミナルでは、テンキーの `Alt+Enter` でも改行が入ります。折りたたまれた貼り付けの隣でも同じです。修飾キーを伴うテンキーの移動キーは、テンキーでない同じキーの動きに従います。
:::
...
| モード | 動作 |
|------|----------|
| `"interrupt"`（既定） | メッセージが動作中のターンの向きを変えます。表示済みの推論と完了した作業を残したまま、モデルの生成をやり直します。前面で動いているターミナルのコマンドは背面へ回され（強制終了はされず、終わったときに通知が届きます）、メッセージがすぐ読まれるようになります。ほかの実行中のツールは先に終わります |
...
どのモードでも、`/queue <prompt>` を使えば、次のターンをはっきり順番待ちに入れられます。`/queue list`、`/queue rm N`、`/queue edit N …`、`/queue move A B` は、実行中であってもすぐに待ち行列へ作用します。何が待っているかは、実況枠に一覧で表示されます。
