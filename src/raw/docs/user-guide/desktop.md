...
- **バーを動かす** — 入力欄のどこかを **少し長めに押し込んで** からドラッグします。さっと押した場合は普通に入力になり、押し続けるとウィンドウをつかみます。HUD を動かす方法はこれだけです。ドラッグ用のタイトルバーはありません。
- **大きさを変える** — バーの右下の角をドラッグします。
- **ポインタに寄せる** — **⌘/Ctrl+Shift+G**（グローバルなホットキーで、どのアプリからでも効きます）を押すと、HUD がカーソルのある場所へ飛んできます。
