...
upstream_blob: 0ab85dd08f955f78115b174ceca29b377cdce9a3
sources:
...
- **設定した場所のもの。** メッセージのチャットで設定した定期確認は、同じセッションを TUI・デスクトップアプリで開いていても、ゲートウェイから流れてそのチャットに返信します。見ている側が時間の到来を横取りすることはありません。
...
- **言うことがなければ黙る（ゲートウェイ）。** 定期確認のターンは、沈黙の印（`NO_REPLY` / `[SILENT]`）だけで終わることがあります。このときゲートウェイは何も送りません。人が送ったメッセージへの返答が印だけで返ってきた場合とは違います（そちらは「もう一度試してください」という案内が見える形で出ます）。定期確認が動いているあいだは、入力中の表示も、ツールの進み具合も、流れてくる下書きも、「まだ作業中です」の吹き出しも出ません。結果を届けるときは、定期確認を設定したメッセージを引用せずにチャットやトピックへ流します。承認を求める問いかけと失敗は、これまでどおり見える形で出ます。
