...
upstream_blob: be1de9f70d2a7b1d7db82399521e59c6fbb5f46a
sources:
...
- **取りまとめ役**は、実際に動くモデルです。参照の出力は、いちばん新しいユーザーのターンの*末尾*に、内輪の案内として足されます。その文は末尾に置かれる — 安定した先頭部分（システムプロンプト + それまでの履歴）より下にある — ので、キャッシュ済みの先頭部分を無効にしません。取りまとめ役は差し込みより上のすべてでキャッシュに当たり、新しく足された末尾だけが新しい分になります。これは普通のターンのふるまいそのもので、新しいユーザーのメッセージもまた、キャッシュに載っていない末尾のトークンです。

というわけで MoA は、どちらの呼び出しでもプロンプトのキャッシュを犠牲にしません。実際の費用は繰り返しごとの参照の呼び出しが増えることだけです — 払っているのは複数のモデルの見方の分であって、壊れたキャッシュの分ではありません。Hermes の他の部分と共有している、長く生きる会話の先頭部分は、そのまま無傷です。
