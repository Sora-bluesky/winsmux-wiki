...
upstream_blob: 72ae9e465b0bb1caf65346a3c82c82d49cecac18
sources:
...
- **取りまとめ役**は、実際に動くモデルです。参照の出力は、内輪の案内だけを載せた*それ自身の*末尾のユーザーメッセージとして足されます — 利用者のメッセージに混ぜ込まれることはありません。その塊は末尾に置かれる — 安定した先頭部分（システムプロンプト + 利用者のメッセージ + それまでのツールの履歴）より下にある — ので、キャッシュ済みの先頭部分をいっさい無効にしません。ツールの繰り返しの中のどのリクエストも、案内の塊を除けば直前のリクエストをバイト単位でそのまま延ばした形になるため、取りまとめ役は差し込みより上のすべてでキャッシュに当たり、新しく足された末尾だけが新しい分になります。これは普通のターンのふるまいそのもので、新しいユーザーのメッセージもまた、キャッシュに載っていない末尾のトークンです。Anthropic Messages、Bedrock Converse、Gemini 純正の通信形式にいる取りまとめ役は、隣り合う2つのユーザーのターンを1つのメッセージにまとめますが、別々のコンテンツブロックとしてです。利用者のメッセージのブロックは、あとの繰り返しが再生するものとバイト単位で同じままで、案内のブロックはその後ろに続くので、キャッシュされた先頭部分は利用者のメッセージのところまで、そのまま通り続けます。
