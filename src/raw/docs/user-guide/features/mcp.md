...
チャットで「Linear の MCP を追加して」と頼むこともできます。エージェントが
`mcp: true` を対象にして `manage_connections` を呼ぶと、設定のカードが表示されます。
このカードは、デスクトップアプリ（ダイアログ）、ターミナルの画面
（`hermes --tui`、入力欄の上に出る案内）、従来の CLI（パネル）のどれでも同じように動きます。

1. **入力欄。** その項目に設定値が決められていれば、カードはそれを一度にすべて表示します。
   ふつうの値には初期値があらかじめ入っています。秘密の値は伏せ字で表示されます。
   入力している間は、まだ何も保存されません。
2. **接続か取り消し。** 取り消すと、そのサーバーだけが飛ばされます。同じ依頼に含まれる
   ほかのサーバーはそのまま進みます。
3. **認可。** OAuth を使う項目では、カードに認可のリンクが表示されます。
   Hermes が勝手にブラウザを開くことはありません。デスクトップでは **Open in browser** を押し、
   ターミナルでは Enter を押します。SSH 越しの場合は、戻り先のポートにつなぐ方法か、
   リダイレクトされた URL を貼り付ける方法をカードが案内します。
4. **保存。** サーバーが新しいトークンを受け入れ、最初の接続が返ってきた時点で、Hermes は
   サーバーの設定、トークン、入力した設定値をまとめて保存します。サーバーがトークンを
   はねた場合や、その前に取り消した場合は、今回の試みの内容は何も残らず、以前の設定と
   トークンは元のままです。失敗したフォームは、入力した内容を残したまま開き直します。
   すでに認可済みのサーバーは、保存してあるトークンで接続します。もう一度認可を求めるのは、
   そのトークンが使えなくなったときだけです。
5. **道具。** そのあと Hermes はサーバーの道具を一覧にして登録します。エージェントは
   同じターンのうちにそれを呼べます。認可はうまくいったのに道具の一覧が取れなかった場合、
   カードには「Authorized. Tools unavailable.」と表示されます。エージェントはあとで、
   認可をやり直させることなく、もう一度道具を探しにいけます。

メッセージアプリにはカードがありません。エージェントが下のコマンドを案内します。
...
**Google が運営するサーバー（Gmail、カレンダー）。** Google は、認可の依頼に `access_type=offline` が付いているときしか更新用のトークンを発行しません。ところが MCP の検出では、この値が案内されることはありません。そこで Hermes は、見つけた認可サーバーが `accounts.google.com` のときに限ってこれを付け加えます（あわせて `prompt=consent` も付けるので、ログインし直したときにも更新用のトークンがもう一度発行されます）。これで接続は再起動をまたいで続き、`hermes gateway` からも使えます。ほかの発行元への依頼には手を加えません。
...
1 回の検出（起動時、`/reload-mcp`、設定の見張り）で同時につなぐサーバーは、最大で **4 つ**です。
stdio のサーバーはそれぞれ子プロセスの木を立ち上げるので、上限がないと、サーバーが多いときに
すべてが同じ瞬間に起動していました。CPU とメモリが跳ね上がり、プロファイルをたくさん動かしている
環境では、プロバイダーへの呼び出しが一斉に集中していました。`config.yaml` で調整できます。
...
上限を超える数のサーバーがあるときは、何回かに分けてつなぎます。それぞれの回にはいつもどおり 120 秒の
持ち時間があり（1 回の検出全体では 300 秒まで）、遅いサーバーが多くても時間切れにはならず、
終わるのが遅くなるだけです。
