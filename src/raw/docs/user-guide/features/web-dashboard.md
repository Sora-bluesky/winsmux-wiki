...
- PTY のソケットに通信がしばらく無いときは、20 秒ごとに小さなリサイズの合図を送ります。そのため、アイドル時間で接続を切るリバースプロキシ（nginx、Caddy）でも、応答を待っているだけのチャットが切れません。一方、接続の寿命そのものに上限を設けるプロキシ（通信の有無にかかわらず約 30 秒でソケットを閉じるトンネルなど）では切れますが、チャットは同じセッションへ自動でつなぎ直します。自動の再接続は、ブラウザのタブが隠れているときや管理画面の別ページを開いているときは止まり、戻ってくると再開します
...
- **ゲートウェイの再起動** — 資格情報は `~/.hermes/.env` に、有効かどうかの印は `config.yaml` に書かれます。ゲートウェイは次に再起動したときに、有効な経路それぞれにつなぎます。その再起動はこのページから直接かけられます。システム全体に入れた場合（`hermes gateway install --system`）は、管理画面が `sudo -n` で再起動をかけるので、管理画面を動かしている利用者にはパスワードなしの sudo が必要です。それがないと、再起動したと報告したあとで CLI に断られるのではなく、その場で要求が失敗します。
