...
キュレーターを呼び出すのは、cron のジョブではなく、手が空いているかどうかの確認です。CLI のセッション開始時、ゲートウェイの片付けの最中、そしてデスクトップや `hermes serve` の手入れ用のタイマーが刻むたびに、Hermes は次の二つを確かめます。

1. 前回のキュレーターの実行から十分な時間が経ったか（`interval_hours`、初期値は **7 日**）
2. エージェントが十分に休んでいるか（`min_idle_hours`、初期値は **2 時間**）

デスクトップやそのほかの `hermes serve` の裏側は、cron のジョブとは別に、もとからある 1 時間ごとの手入れ用タイマーを共有します（最初の確認は 90 秒後です）。手が空いているかどうかは、プロセスが立ち上がった時刻と、同じプロファイルでの直近のやりとりから測ります。セッションが閉じたり片付けられたりしたあとも、そのやりとりの時刻は保たれます。同じプロファイルでひとつの応答が進んでいる間は、キュレーターを飛ばします。つながっているだけで何もしていない窓が、手入れを止めることはありません。このタイマーは個人と組織のスキル同期の確認もしますが、それぞれの機能を自分で有効にしている場合に限ります。同じプロファイルのメッセージ用ゲートウェイが動いているときは、これらの片付けはそちらが受け持ちます。

タイマーが世話をするのは、その裏側が持つプロファイルです。進行中の手入れは作業用のスレッドで走り、裏側を閉じてもその一巡が途中で止まるわけではありません。同じプロファイルに対して独立した serve のプロセスをいくつも立ち上げると、キュレーターの間隔の確認が競合することがあります。
...
2. **LLM によるまとめ直し**（補助モデルを 1 回だけ使い、反復の上限は高めです。ひととおりの手入れで API 呼び出しはだいたい 50〜100 回になります）— こちらは**初期状態では切ってあります**。`curator.consolidate: true` にすると、分身のエージェントがエージェント作のスキルを見渡し、必要なら `skill_view` でどれでも読み、スキルごとに、そのまま残す／`skill_manage` で手を入れる／重なっているものを種類ごとの傘にまとめる／書庫へ移す（`skill_manage action=delete` に `absorbed_into` で移し先を添えます。この段階にターミナルは渡されません）、を決めます。渡される候補の一覧には、実際に読み書きできるスキルだけが入ります。同梱の組み込みスキル（`prune_builtins: true` にしていても同じで、この設定が効くのは決まりきった書庫入りの処理だけです）と、`skills.disabled` に入れたスキル（`skill_view` が読み込みを断ります）はそもそも候補に出てこないので、断られる読み書きでツールの回数を使い切ってしまうことがありません。まとめ直しでは、スキルを一式のまとまりとして扱います。スキルが `references/`、`templates/`、`scripts/`、`assets/` を持っていたり、それらへの相対リンクを持っていたりする場合、キュレーターは単独のまま残すか、必要な補助ファイルを引っ越させてパスを書き換えるか、その一式ごと手つかずで書庫へ入れるかのどれかを選ばなければなりません。`SKILL.md` だけを別のスキルの `references/` に平たく押し込むことは許されていません。
