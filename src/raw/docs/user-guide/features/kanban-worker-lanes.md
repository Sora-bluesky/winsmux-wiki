...
### 派生したプロセスの範囲 {#descendant-process-scope}

仕事の割り当ては、割り振り役の作業役に属するものであって、そこから起動されるすべてのプログラムに
広がるわけではありません。Hermes の子プロセス用の補助は、シェル、実行カーネル、cron の配信、フック、
言語サーバー、ふつうの stdio 型の MCP サーバーへ、「持ち主ではない」という囲いを引き継ぎます。
そこからさらに生まれた子も、途中のスクリプトが受け取った仕事の ID を消したところで囲いの中に
...
ふつうのシェルの子は囲いの中に残ります。作業役ができるのは、一生の受け渡しと、自分に割り当て
られた仕事へのファイルの添付だけです。`unblock` は取りまとめ役だけのものです。別の仕事への
コメントと、追いかけの仕事の作成は、これまでどおりの動きです。

連携を書く人がプロセスを起こすときは、環境変数の上書きをまとめたあと、実際に起こすその場所で
`agent.delegation_context.delegated_child_subprocess_env` を使ってください。呼び出した側の
資格情報とプロファイルの決まりがそのまま保たれます。これは動いている最中の取り決めによる範囲分けで
あって、**OS のレベルで閉じ込めるものではありません**。素性を示す情報をわざと消したり、SQLite を
直に開いたりするコードを止められるわけではありません。
