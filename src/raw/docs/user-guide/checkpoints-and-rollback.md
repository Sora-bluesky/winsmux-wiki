...
- **保管場所全体の上限** — 保管場所が `max_total_size_mb`（既定は 500 MB）を超えると、プロジェクトを順番に回りながら、それぞれのいちばん古いコミットを落とします。1 つ落とすたびに実際に容量を取り戻してから、もう 1 つ落とす必要があるかを判断します。どのプロジェクトにも、スナップショットが少なくとも 1 つは残ります。Git の操作が失敗すると掃除はそこで止まり、失敗したことが報告されます。容量を取り戻せなかった分を埋め合わせるために、さらに履歴を捨てることはしません。
...
- **同時に走る操作** — スナップショット、復元、差分の表示、保守の作業は、プロセスをまたいで共有する 1 つの保管場所ロックを使います。ほかのプロセスがまだ確定させていないオブジェクトに対して GC を走らせることはせず、保管場所が使用中だと報告します。復元では、選んだツリーを当ててから、安全のために取ったスナップショットの履歴を掃除します。
- **変更がないときのスナップショット** — 前回のスナップショットから何も変わっていなければ、チェックポイントは飛ばされます。
- **エラーは致命的ではありません** — スナップショットに失敗しても、ツールは止まりません。掃除の失敗は警告として記録されます。明示的に実行した保守の作業では、エラーの件数が報告されます。
...
隣にある `.checkpoints.lock` はプロセス同士の調整に使われ、保管場所を消去しても残ります。Hermes が動いている間は消さないでください。
