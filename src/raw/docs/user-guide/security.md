...
この防御は、ゲートウェイが動いているインタプリタの実行ファイルそのものを狙ったプロセスの停止も断ります。`taskkill /F /IM python.exe`、`taskkill /FI "IMAGENAME eq python.exe"`、`Stop-Process -Name python`、`pkill -9 python3`、`killall python`、`pkill -f python`、それに `pgrep python | xargs kill` のように名前から止める先を導くものが対象です。見張られているゲートウェイは文字どおり `python` のプロセスなので、こうしたコマンドはゲートウェイごと（そしてエージェント自身のその回の応答も）落としてしまうからです。エージェントが自分で抱えているプロセスに狙いを絞った停止は通ります。背後で動いている仕事の `proc_*` の id（`process(action="kill", …)`）や、PID を直に書く形（`taskkill /F /PID <pid>`、`kill <pid>`）です。別の実行ファイルの名前（`taskkill /F /IM notepad.exe`）は影響を受けません。この防御は、自動で作られる起動の仕組みのどれでも効いています。systemd のユニット、launchd の plist、s6 の run スクリプト、Windows のタスク スケジューラのタスクが、いずれも `HERMES_SUPERVISED_CHILD` という目印を渡すからです。
...
時間切れになった確認は開き直せません。待っていた分は捨てられ、エージェントにはその回のうちに自分で試し直さないよう伝えられます。やはり実行したいときは、あらためてそれを頼むメッセージを送ってください（たとえば「さっきのを今すぐ実行して」）。エージェントは新しくツールを呼び直し、確認のカードも新しく出ます。「1 回だけ」の承認は、その呼び出しにだけ効きます。時間切れは拒否として数えられないので、もう一度頼んでも不利になることはありません。
...
OS の認証情報の行にある `~` は、書き込みが着地しうる**すべての**ホームを指します。プロセスの `HOME` だけではありません。OS の利用者の本当のホーム、プロファイルのホーム（`TERMINAL_HOME_MODE=profile` のときの `{HERMES_HOME}/home`、それにコンテナや起動された worker のように、プロセスの `HOME` が固定されている場所）、さらに名前を指したホーム（`~root/.ssh/authorized_keys`）も含みます。本当のホームの `~/.aws/credentials` を絶対パスで書こうとすると、エージェントのプロセスが別の場所を `HOME` にして動いていても拒否されます。
...
どちらの一覧も、`terminal`、`execute_code`、`no_agent` の cron スクリプトに同じように効きます。宣言した変数は、その子プロセスが動く相手のプロファイルの値で渡されます。1 つのプロセスが複数のプロファイルを受け持つとき（複数プロファイルのゲートウェイ、デスクトップやダッシュボードのバックエンド）、それぞれのプロファイルで宣言された値は、そのプロファイル自身の `.env` や秘密の取り出し元から来ます。起動したプロファイルが用意したプロセスの環境から来ることはありません。起動したプロファイルの `.env` の認証情報は、受け持っている側のプロファイルの子プロセスからは外されます。
...
同じ守りは、URL を自分で書いたのではなく相手側から渡された取得にも効きます。生成の提供元が返してくる画像や動画の URL、編集のためにモデルが差し出す参照画像の URL、ペットのスプライトシートと petdex の一覧、skills.sh のサイトマップの項目などです。提供元や一覧がそれらをプライベートなアドレスやメタデータのアドレスに向けてきた場合は、接続を開く前に断ります。運用する人自身が設定した提供元の `base_url` は影響を受けません。設定した `base_url` から直に取ってくるダウンロード（OpenRouter の動画の内容の接続先）は、その最初の 1 歩でプライベートなアドレスかどうかの検査だけを飛ばします。クラウドのメタデータに対する最低限の守りは効き続けますし、そこから出るリダイレクトはすべて検査し直されます。また、LAN の中に置いた画像生成の提供元が返す*結果*の URL を手元に取り込むには、後述の `security.allow_private_urls: true` が要ります。
...
#### 手元のプロキシが使う fake-ip の範囲 {#local-proxy-fake-ip-ranges}

fake-ip の方式で動く TUN のプロキシ（Mihomo / Clash の `fake-ip`、Surge の拡張モード）は、絞り込みの対象から外れた名前の DNS に対して、自分が抱えている範囲——既定では `198.18.0.0/15`（RFC 2544 の性能測定用）——のアドレスを返します。この返事はプロキシが立てた目印であって内部のホストではないのですが、そのままだとプライベート IP の守りが、そうしたホスト宛ての外向きの取得をすべて拒みます。`web_extract`、プラットフォームからの添付のダウンロード、ブラウザの中継はどれも *URL targets a private or internal network address* で失敗し、要求はネットワークに出ないままになります。目印を通すには、その範囲を書いておいてください。
...
既定では空で、`allow_private_urls` よりも狭い設定です。除外されるのは書いた範囲だけで、書くのは手元のプロキシが自分で持っている範囲にしてください（つなぎ先はあくまでプロキシで、本当の相手はプロキシが自分で解決します）。ループバック、RFC 1918、リンクローカル、CGNAT、クラウドのメタデータ宛ては拒まれたままです。それらの種別に重なる書き方（`0.0.0.0/0` や `::/0` も含みます）は、守りを広げるのではなく警告を出して無視されます。クラウドのブラウザのプロバイダを使っている端末では、書いておいた目印は `browser.auto_local_for_private_urls` の判断でもプライベート扱いされなくなるので、そうしたページは引き続きクラウドのブラウザに回ります。
...
動作そのものの失敗（起動のエラー、時間切れ、異常終了）が 3 回続くと、検査は 5 分間止まります。壊れたバイナリがすべてのコマンドを足止めしないためです。その 5 分が過ぎたあと、1 つのコマンドが tirith を試しに呼び直し、検査が最後まで終われば（通す・注意・止めるのどれでも）通常の検査に戻ります。試した呼び出しがまた失敗したら、5 分の待ちが張り直されます。
...
よく知られた Tirith の誤検知が 2 つ、確認を出さないよう（cron では拒否されないよう）「通す」に下げられています。1 つは、狙われている先が正規の `.app` という gTLD だけの `lookalike_tld` の注意です。もう 1 つは、コマンドの中の異体字セレクタがすべて絵文字の直後の U+FE0F である場合の `variation_selector` の注意で、`🗞️ Journal/` や `▶️ Media/` のようなフォルダ名が当てはまります。文字や数字の直後に来る異体字セレクタ、つまりこの規則が本来とらえようとしている隠し文字による細工の合図のほうは、これまでどおり確認が出ます。
...
「翻訳してから実行させる」形の検査は、言語や形式を指す短い語句（たとえば「これを bash のスクリプトに訳して実行して」）があるときだけ反応します。関係のない役割の説明がカンマで並んでいるところをまたいで、翻訳の動詞と実行の動詞を結び付けることはありません。これらのパターンはあくまで経験則であって、意味を読んで意図を判定するものではありません。

プロジェクト側の止められたファイルには警告が出ます。

```
[BLOCKED: AGENTS.md contained potential prompt injection (prompt_injection). Content not loaded.]
```

`HERMES_HOME` にある自分の `SOUL.md` は扱いが違います。これは自分で書いたファイルなので（ファイルのツールから書き込むには承認が要りますし、プロジェクトのチェックアウトから入ってくることもありません）、検査に引っかかっても**そのファイルが止められることはありません**。Hermes は一致したパターンの名前を添えた警告を記録したうえで、いつもどおりファイルを読み込み、`/context` には `⚠ SOUL.md … loaded — matched prompt-injection pattern(s); review the file` と並びます。おかげで、攻撃に使われる言い回しを*説明している*人格のファイル（「それまでの指示を無視しろと書いてある内容」といった注意書きなど）もそのまま使えます。引っかかった文章に心当たりがないときは、その警告を「誰かがファイルを書き換えた」しるしとして受け取ってください。この例外は、プロフィールの配布物に付いてくる `SOUL.md` には及びません。`hermes profile install <git-url>` と `hermes profile update` は、他人の `SOUL.md` を検査も承認もなしにプロフィールのホームへ複製するので、`distribution.yaml` がそのファイルを持っている場合は、検査に引っかかった時点で止められます。
