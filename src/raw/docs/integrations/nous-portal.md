...
[Hermes のプロファイル](/hermes/docs/user-guide/profiles/)を使っている場合、Portal のリフレッシュトークンは共有のトークン保管場所を通じて、すべてのプロファイルで自動的に共有されます。どれか 1 つのプロファイルで一度ログインすれば、ほかのプロファイルも自動でそれを使うので、プロファイルごとに OAuth の手順をくり返す必要はありません。
