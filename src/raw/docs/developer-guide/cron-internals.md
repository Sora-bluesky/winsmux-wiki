...
6. **一時停止中・無効・終わった状態のジョブは動きません。** 実行時刻の走査は、上のどれよりも前に
   それらを外します。一時停止や再開をすると、残っている予定の刻印も消えます。ただし、*一時停止している間に*
   実行時刻が来た繰り返しの回は失われません。`resume_job` は、保存されている `next_run_at` が
   過去であればそれを実行時刻としてそのまま残し、いまを起点に取り直すことはしません（残したことも
   ログに記録します）。そのため再開後の最初のティックで、その回に規則 3〜5 が当てはまります。
   遅れての実行かまとめての実行が 1 回起きるか、ログに残る形で飛ばされるかのどちらかです。
   1 回きりのジョブと、未来の時刻は、再開時にいまを起点に計算し直します。

どの構成でも、同じ保存の項目が動きを決めます。単独の `hermes -p X gateway
run` も、ホストのゲートウェイが受け持つプロファイル（`_start_multiplex` は
それぞれのホームを `_profile_cron_scope` の下でティックします）も、まったく同じ記録を評価します。
ホストごとに 1 つのゲートウェイのプロセスが、*すべての*プロファイルの保存場所をティックします
（`gateway.multiplex_profiles` が切り替えるのはアダプターで、cron ではありません）。実行ごとの管理
（進行中の権利の確保、並列ワーカーのプール、古いコードのときに譲るかどうかの判定）はプロファイルの
ホームごとに分けて持つので、2 つのプロファイルに同じ名前のジョブがあってもぶつかりません。
自分のゲートウェイを動かしているプロファイルはティックのたびに飛ばされるので、2 つのプロセスが
その保存場所を取り合うことはなく、配信も必ずそのプロファイル自身の動いているアダプターから出ていきます。

**実行中は発火の権利を借り受けます。** 発火した実行は `fire_claim = {at, by}` を持ち、鼓動を打つスレッドが
60 秒ごとに `at` を更新し続けます（借り受けの期限は 300 秒です）。鼓動の見張りが、その権利を別の誰かのものだと
読み取っても、それだけでは数えず、もう一度読み直します。取られたことが確かめられたときにだけ、進行中の実行を
取り消します。それでもなお、実行の結末はその場の判定ではなく、終わった時点の保存内容に照らして決まります。
保存側がまだその権利を認めるなら、実行の本当の結果（`ok`、または実際のエラー）が記録され、本当に
持ち主が変わっていたときは古い結果を捨て、新しい持ち主の記録に上書きすることはありません。
`Interrupted by shutdown before terminal completion.` が記録されるのは、権利を持ったままの実行が
本物の通信の取り消し（ゲートウェイの排出）で止められたときだけです。
...
動いている最中にチェックアウトが更新されたティッカー（起動時のリビジョン ≠ ディスク上のリビジョン）が
ティックを譲るのは、実際に引き継げるゲートウェイに対してだけです。実行時のロックを持っているのが
生きているゲートウェイで、その `gateway_state.json` の鼓動が新しく、記録された `code_sha` が
ディスク上のリビジョンと一致していなければなりません。ロックを持っているプロセス自身がまだ更新前の
コードで動いている場合（ゲートウェイが 1 つだけの環境で `hermes update` した直後によくある状況です）は
新しいゲートウェイとは見なさないので、ティッカーは誰もいない相手に毎回ティックを譲るのではなく、
そのまま実行を続けます。
...
### 古いコードのときにティックを譲る {#stale-code-yield}

ティックのロックを取る前に、動いている最中にチェックアウトが更新されたゲートウェイ（起動時のリビジョン ≠ ディスク上のリビジョン）は、別のプロセスがゲートウェイの実行時ロックを持っていればティックを譲ります。より新しいゲートウェイのティッカーが代わりに実行し、古いほうは `sys.modules` が混ざった状態でそれと競ってはいけないからです。譲ったことは例外（`CronTickYielded`）として送出され、ティッカーの最後のエラーとして保存されます。そのため、生存確認の鼓動が更新され続けていても、`hermes cron status` は両方のリビジョンと再起動のコマンドを添えて **"Gateway is running STALE code — its cron ticker yields every tick and fires NOTHING"**（ゲートウェイが古いコードで動いていて、cron のティッカーが毎回ティックを譲り、何も実行していない）と報告します。この流れを閉じるのが `hermes update` です。更新後に各ゲートウェイの版を突き合わせた結果、古いと確かめられたゲートウェイは、動かしたままにされるのではなく、処理を片付けてから再起動する `request_restart` の経路（SIGUSR1）へ渡されます。監視下のゲートウェイは新しいコードで起動し直し、素の `gateway run` は停止されて「Restart manually」の一覧に載ります。
