...
環境変数は `gateway.platforms._shared.get_scoped_secret` を通して読んでください。`os.getenv` は使いません。`gateway.multiplex_profiles` のもとでは、プロセスの環境変数には既定（DEFAULT）のプロファイルの値が入っています。ほかのプロファイルの `.env` はそのプロファイルの秘密の範囲の中にしか無いので、そのまま読むと、違うプロファイルで、違う認証情報のまま、あなたのプラットフォームを有効にしてしまいます。`seed_extra_from_env` は、`(ENV_VAR, extra_key, conv)` の表から、その読み取り口を通して用意用の dict を組み立てます。
