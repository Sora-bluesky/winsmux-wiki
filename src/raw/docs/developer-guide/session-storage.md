...
名前付きのプロファイルは、それぞれ自分の `$HERMES_HOME/state.db` に会話の記録を持ちます。1 つの
`hermes serve` プロセスが複数のプロファイルを受け持っている場合も同じです。セッションの途中で
エージェントを組み直すとき（Bot Chat の機能更新と `tools.configure`）は、そのセッションの
データベースハンドルを保ったまま、組み立ての間そのプロファイルのホームに結び付けておく必要があります。
役目を終えるエージェントを解放するときに、後継が引き継いだハンドルを閉じてはいけません。
`tools.configure` は、クライアントが `session_id` しか渡していないときでも、動いているセッションの
`profile_home` から設定を解決します。組み直しでは、後継を確保する前にモデル設定を用意し、そのうえで
エージェントを据えて所有権をまとめて移します。用意の段階で失敗した場合、後始末の責任は既存の
エージェントに残ります。明示されたプロファイルが解決できない、あるいはそのディレクトリが消えている
場合は、起動設定や履歴に触れる前に失敗します。古くなった `tools.configure` のセッション ID も同様に、
設定を変えずに `session not found` を返します。セッション ID を省いたときは、これまでどおり全体設定の
操作として扱われます。

その場での圧縮では、古い行を `active=0` にして保管し、残したコンテキストを `active=1` の行として
入れ直します。そのため、保護されたメッセージが、同じ内容・同じ日時のまま両方の世代に現れることが
あります。これは正しい状態なので、重複と見なして保管側の行を消さないでください。実際に動いている
書き込みの重複を調べるときは `active=1` を使い、履歴が巻き戻ったように見えるときは、セッション ID
だけでなくデータベースのプロファイルも確かめてください。

## Codex app-server における入力の持ち主 {#codex-app-server-input-ownership}

エージェントは、Codex のターンを始める前に、受け付けた利用者の入力を保存します。Codex はそのあと、
同じ入力を先頭の `userMessage` 通知として流してきます。実行時のつなぎ目では、Hermes は `turn/start`
に書き出したテキストと完全に一致する場合にかぎって、その先頭の項目だけを除きます。リッチな入力を
変換したものも一致の対象です。後から来た利用者イベントや、一致しないイベントはそのまま残り、別々に
受け付けられた同じ内容のターンも残ります。合成された入力やキーを持たない入力にも同じ扱いが適用され、
プラットフォーム側のメッセージ ID には依存しません。すでに履歴にある重複を書き換えることはしません。
エージェントが「保存は自分が持っている」と伝えた場合、ゲートウェイは会話記録の書き込みを行いません。

## ゲートウェイの例外経路における入力の持ち主 {#gateway-exception-path-input-ownership}

ゲートウェイの例外は、エージェントを組み立てる前に起きることもあれば、その入力が SQLite に届いた
あとに起きることもあります。ゲートウェイは受け付けた入力に、既存の `display_metadata` の付随領域を
使って持ち主の目印を付け、エージェントのふだんの保存経路へ渡します。プロバイダへ送るメッセージに
このメタデータが入ることはありません。プラットフォームの目印は、受信メッセージ ID をプラットフォーム・
プロファイル・スコープ・チャット・スレッドで区切って一意にします。引用や返信の解決に使う
`platform_message_id` は元のまま変わりません。キーを持たないターンには、テキストと日時がまったく
同じでも新しい目印が振られます。

例外時の書き込み側は、その目印だけを探します。探す順番は、公表された転送先、正規の現行圧縮の後継、
そのあと圧縮の祖先です。数えるのは有効な行と圧縮の保管分で、取り消された行、観測された入力、
無関係な書き込み手は数えません。同じセッションに書き込んでいる無関係なプロセスが、このターンを
消してしまうことはありません。履歴全体の基準を取ったり、保管されたメッセージ本文を読み込んだりする
必要もありません。持ち主の読み取りに失敗したときに、当て推量で追記することは許されません。ふつうの
履歴読み取りの失敗は、これまでどおり「履歴を取得できない」という応答になります。

エージェントが持ち主になる通常の保存の流れは変わりません。これは失敗したときの書き込み手を裁くための
仕組みであって、あらゆる場合に 1 回きりの配送を保証するものでも、内容の重複を取り除くものでも、
スキーマの移行でもありません。過去の行を書き換えることはなく、目印のない過去の入力が、再配送された
イベントの持ち主を示す根拠になることもありません。
