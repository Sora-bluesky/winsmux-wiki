...
`tools/process_registry_checkpoint.py` は、動作中プロセスのチェックポイントと、
PID を取り違えないプロセスの引き取りを受け持ちます。終了したプロセスの出力は別扱いで、
`tools/process_registry_results.py` がプロファイルの `logs/process-results/` の下に、
1 プロセスにつき 1 通の受領記録を、秘匿処理をかけたうえで一括で書き込みます。
共有の PID チェックポイントを書き換えて別の親の結果を上書きすることはできません。
レジストリは完了イベントを解放する前に受領記録を保存し、一度きりの待ち受けはそのイベントを待ちます。
既存のプロセス照会メソッドは、保持されたスナップショットを読むだけで、PID を引き取ったり
通知を積んだりはしません。読み出しには、そのプロセスを起こした永続セッションか、その圧縮後の
継続セッションが必要です。ハンドルを知っているだけでは、保持された結果を読む権限にはなりません。
レジストリは出力の読み取りを始める前に持ち主を控えます。CLI から起動したプロセスや通知を出さない
プロセスでも同じで、読み取りスレッドには生成元のプロファイル情報が引き継がれます。
受領記録の秘匿処理は、実行中の出力を見せない設定とは無関係に必ずかかります。保持は日数と件数で
上限が決まります。
