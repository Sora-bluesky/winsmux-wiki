...
upstream_blob: 5859174c7f7e1e68e9e3cc08224aed37754967ec
sources:
...
ツールの動作、worktree の試行、手動のチェックアウトなどで、ソースのチェックアウトが機能ブランチに置き去りになっている場合、作業ツリーがきれいでありさえすれば、`hermes update` はそのチェックアウトを更新対象へ自動で戻します。

- **ブランチが完全にマージ済みの場合**（コミットがすべて `origin/main` に含まれていて、`git cherry` が未マージのものを報告しない）: 更新はその旨を `Checkout was parked on '<branch>' (fully merged) — switched back to main` と表示し、以降は `main` にとどまります。
- **ブランチに未マージのコミットがある場合**でも、ツリーがきれいなら更新は `main` へ切り替えて処理を進めます。デスクトップの更新ボタン、ゲートウェイの `/update`、cron といった非対話的な呼び出し元は、スキップされても対処のしようがないため、この動作を前提にしています。コミットには手が触れられません。`git checkout` はコミット済みの作業を捨てることがなく、更新はブランチ名とコミット数を示す目立つ通知を出し、あとで作業を再開するための `git checkout <branch>` コマンドも一緒に表示します。

main の上に自分のパッチを重ねて維持しているなど、*意図して*独自のブランチで運用している場合は、`config.yaml` に `updates.parked_branch_strategy: update_in_place` を設定してください。更新はブランチから離れる代わりに、`origin/main` を自分のブランチ**へ**マージします。チェックアウトは動かず、コミットも残ったまま、動作するコードだけが前へ進みます。可能なときは fast-forward で、履歴が分かれている場合は `pre-update-<stamp>` という安全用のタグを付けたうえで本来のマージを行い、衝突したときは何も変えずにきれいに停止します。`hermes update --switch-branch` を使うと、その 1 回だけ切り替える動作に戻せます。更新由来のマージコミットを積みたくない、深く分岐した機能ブランチで役立ちます。

置き去りのブランチにコミットしていない変更がある場合（ツリーが汚れている場合）、Hermes はそのブランチに**手を触れません**。コードの更新は **SKIPPED** として扱われ、対象のブランチ名、`origin/main` からどれだけ遅れているか、解消するための正確なコマンドを示す目立つ警告が出ます。更新が成功したふりはしません。完了行には常に実際のブランチと HEAD が表示されるので（`✓ Update complete! [main @ 30fcf9580]`）、ずれがひと目で分かります。自動の切り替えを完全に止めたい場合は、`config.yaml` に `updates.auto_switch_parked_branch: false` を設定してください（スキップ時の警告は引き続き出ます）。
...
### 全体の下見: `hermes update --plan` {#fleet-preview-hermes-update---plan}

プロファイルやサービスを複数動かしている端末を更新する前に、`hermes update --plan` を実行すると、何も変更しないまま更新の計画を丸ごと表示できます。インストールの種類（git のチェックアウト、Docker イメージ、Nix / apt による管理）、全プロファイルにまたがって稼働している Hermes のサービスと、その管理方式（systemd、launchd、手動）、実際に動かしているコードのバージョン、そしてそれぞれに適用される再起動の方法が並びます。イメージやパッケージで管理されているインストールでは、その場では更新できない旨と、正しい更新コマンドが示されます。読み取りだけなので、稼働中の環境に対しても安全に実行できます。

同じ一覧は実際の更新を行うたびに控え（`~/.hermes/logs/update_receipts/`）へも埋め込まれるため、更新後に「更新処理が何を見て、何をしたのか」を突き合わせられます。

### 更新の控えとバージョンの照合 {#update-receipts-and-the-fleet-version-check}

`hermes update` は実行のたびに、機械で読める控えを `~/.hermes/logs/update_receipts/` に書き出します（直近 20 件を保持し、`latest.json` は常に最新のものを指します）。控えには、更新前の計画、実行した各手順、スキップした内容とその理由、ゲートウェイ再起動の結果、最終的なバージョンの一覧が入ります。再起動の段階が終わると、更新処理は稼働中の各ゲートウェイが動かしているコードを、更新したてのチェックアウトと比べ、プロファイルごとの一覧を表示します。更新前のコードを配り続けているゲートウェイがあれば、正確な再起動コマンドとともにはっきり報告され、終了コードも 0 以外になるので、バージョンの混ざった状態が自動処理から健全だと見なされることはありません。
