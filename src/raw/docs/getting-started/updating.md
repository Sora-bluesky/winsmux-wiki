...
upstream_blob: 24c9efde0efc599261e905123c0d21bff2e6f433
sources:
...
前回の更新がコードの取得までは終えたのに再起動をやり切れていない場合、次の
`hermes update` は、チェックアウトがすでに最新でも再起動をやり直します。プロセスが
見つからないことは、復旧できた証拠にはなりません。失敗した systemd のユニットや、
登録済みの launchd のジョブには、動いている PID が無いこともあるからです。管理役を
見つけられない、再起動に失敗した、あるいは対象のサービスが動いていると確認できない
ときは、再起動待ちの印をそのまま残します。更新は 0 以外で終了して対象のサービスを
報告するので、表示されたコマンドで復旧させてから `hermes update` をやり直してください。

過去のレシートが失敗のまま残っていても、それだけでゲートウェイが古いままだとは言えません。起動時とゲートウェイ状態の警告、そして更新の追いつき処理は、レシートだけを根拠に再起動へ動く前に、動作中の状態を確かめます。記録されたゲートウェイのプロファイルには、いずれも現在のチェックアウトで動く後継が必要です。無関係な最新のゲートウェイが、見当たらない・止まっている・バージョン不明・ゲートウェイ以外のプロセスの代わりを務めることはできません。そのため、手動でゲートウェイを起動し直せば、失敗した更新を成功として書き換えることなく警告を解消できます。再起動待ちの印が別にある場合は、そちらが優先されます。一覧がレシートまで届かなかった、より新しい中断済みの更新のものかもしれないからです。
